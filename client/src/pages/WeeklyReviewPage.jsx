...
const WeeklyReviewPage = () => {
  ...
  return (
    <AppLayout>
      <div className="space-y-8 max-w-5xl mx-auto px-2 sm:px-0 pb-10">
        
        <section className="relative overflow-hidden rounded-3xl border border-zinc-200 dark:border-zinc-800/60 bg-gradient-to-br from-white to-zinc-50 dark:from-zinc-900 dark:to-black p-8 shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-left">
              <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 tracking-widest uppercase mb-2">Weekly Reflection</p>
              <h1 className="text-3xl sm:text-4xl font-bold text-black dark:text-white mb-2">Your Week in Review</h1>
              <p className="text-zinc-600 dark:text-zinc-400 max-w-md mx-auto md:mx-0">
                Look back at the last 7 days of consistency, {user?.username}. This is how you're building your future.
              </p>
              <Button 
                onClick={() => setShareOpen(true)}
                className="mt-6 w-full sm:w-auto bg-zinc-100 dark:bg-zinc-900/50 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 backdrop-blur-sm gap-2"
              >
            ...
            <div className="shrink-0 text-center">
              <div className="relative w-32 h-32 rounded-full flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-200 dark:border-zinc-800 mx-auto shadow-2xl shadow-emerald-500/20">
                <svg className="absolute inset-0 w-full h-full -rotate-90">
                  <circle cx="64" cy="64" r="60" fill="none" className="stroke-zinc-200 dark:stroke-zinc-800" strokeWidth="6" />
                  <circle 
                    cx="64" 
                    cy="64" 
                    r="60" 
                ...
                <div className="flex flex-col items-center">
                  <span className="text-3xl font-black text-black dark:text-white">{data.averageScore || 0}%</span>
        ...
        <div>
          <h2 className="text-xl font-semibold text-black dark:text-white mb-4 flex items-center gap-2">
            <Calendar className="w-5 h-5 text-emerald-500 dark:text-emerald-400" /> Daily Breakdown
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-7 gap-3">
            {data.dailyPerformance.map((day, i) => {
              ...
              return (
                <div key={i} className={`relative flex flex-col items-center p-4 rounded-2xl border ${isBest ? "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.15)]" : "bg-white dark:bg-zinc-900/50 border-zinc-200 dark:border-zinc-800/60"}`}>
                  ...
                  <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-1">{dayName}</span>
                  <span className="text-2xl font-bold text-black dark:text-white mb-3">{day.score !== null ? `${day.score}%` : "-"}</span>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="bg-white/80 dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800/60 backdrop-blur-md">
            <CardHeader>
            ...
            <CardContent>
              {data.topSkipped.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  <div className="w-12 h-12 rounded-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center mb-3">
                    <Sparkles className="w-6 h-6 text-emerald-500 dark:text-emerald-400" />
                  </div>
                  <p className="text-zinc-700 dark:text-zinc-300 font-medium">Perfect execution!</p>
                ...
                <div className="space-y-4">
                  {data.topSkipped.map((habit, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800">
                      <div className="flex items-center gap-3">
                        <div className="w-2.5 h-2.5 rounded-full shadow-sm" style={{ backgroundColor: habit.color }} />
                        <span className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{habit.title}</span>
                      </div>
          ...
          <Card className="bg-white/80 dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800/60 backdrop-blur-md">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-black dark:text-white flex items-center gap-2">
                <Flame className="w-5 h-5 text-orange-500 dark:text-orange-400" /> Weekly Highlights
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-transparent border border-emerald-500/20">
                ...
                <div>
                  <p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Consistency Score</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">You achieved a {data.averageScore || 0}% average completion rate across all your scheduled habits this week.</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-blue-500/10 to-transparent border border-blue-500/20">
                ...
                <div>
                  <p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">Peak Performance</p>
